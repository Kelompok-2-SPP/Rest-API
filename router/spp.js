...
app.put("/", async(req, res) => {
    if (req.body.id_spp) {
        ...
        await spp.update(data, { where: { id_spp: data.id_spp } })
            .then(scss => {
                if (scss[0]) {
                    spp.findOne({ where: { id_spp: data.id_spp } })
                        .then((resu) => {
                            res.status(200).json({
                                status: res.statusCode,
...
app.delete("/", async(req, res) => {
    if (req.query.id_spp) {
        await spp.findOne({ where: { id_spp: req.query.id_spp } }).then(resu => {
                if (resu) {
                    spp.destroy({ where: { id_spp: req.body.id_spp } }).then(
                        res.status(200).json({
                            status: res.statusCode,
