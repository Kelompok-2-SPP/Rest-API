...
app.post("/", async(req, res) => {
    if (req.body.nama_kelas && req.body.jurusan && req.body.angkatan) {
        ...
        await kelas.findOne({ where: { nama_kelas: data.nama_kelas } }).then(
            duplicate => {
                if (!duplicate) {
                    kelas.create(data)
                        .then(result => {
...
app.put("/", async(req, res) => {
    if (req.body.id_kelas) {
        ...
        await kelas.findOne({ where: { nama_kelas: data.nama_kelas } }).then(find => {
            if (!find) {
                kelas.update(data, { where: { id_kelas: data.id_kelas } })
                    .then(scss => {
                        if (scss[0]) {
                            kelas.findOne({ where: { id_kelas: data.id_kelas } })
                                .then(resu => {
                                    res.status(200).json({
                                        status: res.statusCode,
...
app.delete("/", async(req, res) => {
    if (req.query.id_kelas) {
        await kelas.findOne({ where: { id_kelas: req.query.id_kelas } }).then(resu => {
            if (resu) {
                kelas.destroy({ where: { id_kelas: req.query.id_kelas } })
                    .then(
                        res.status(200).json({
                            status: res.statusCode,
